...
export class Heap {
  ...
  */
  ...
  constructor(fh: ArrayBufferLike, offset: number) {
    ...
    let heap_data = fh.slice(
      data_offset,
      data_offset + local_heap.get('data_segment_size')
    );
...
export class GlobalHeap {
  ...
  */
  ...
  constructor(fh: ArrayBufferLike, offset: number) {
    ...
    let heap_data_size =
      header.get('collection_size') - GLOBAL_HEAP_HEADER_SIZE;
  ...
  get objects() {
    ...
    if (this._objects == null) {
      ...
      while (offset <= this.heap_data.byteLength - GLOBAL_HEAP_OBJECT_SIZE) {
        ...
        let obj_data = this.heap_data.slice(
          offset,
